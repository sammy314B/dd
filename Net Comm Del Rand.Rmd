...
#This installs the other packages required to run the program besides igraph, which cannot run normally in this environment and is installed manually.
...
edgelist <- read_csv("randfile.csv", col_names = c("from", "to"))
...
```{r , fig.width= 13 , fig.height = 12}
### Let us also check the individuals that would be identified as important to remove in order to fracture the network by standard centrality metrics
...
node_deletion_of_highdegree <- delete_vertices(igraphobject, V(igraphobject)$name %in% c("Leader Y", "Leader X", "Actor 11", "Actor 14", "Actor 10"))
set.seed(1234)
