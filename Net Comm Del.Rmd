...
edgelist <- read_csv("karate_el.csv", col_names = c("from", "to")) # !MANUAL:Input any file in an edgelist format here.

## Try this with karatedirected_el.csv (Directed graph) and randfile.csv (random dataset)
...
igraphobject <- graph_from_data_frame(edgelist, directed = FALSE) # !MANUAL Change directed = TRUE for a directed graph. 
...
clustered <- cluster_walktrap(igraphobject) # !MANUAL replace cluster_walktrap(igraphobject) with cluster_infomap(igraphobject) if graph is directed.
...
results <- keyplayer::kpset(as_adjacency_matrix(igraphobject), size = 5 # !MANUAL Decide how many keyplayers you want
                            , type = "fragment"
                            , T = 1, binary = T)
...
layouts <- layout_with_fr(node_deletion_of_keyplayers)
...
clustereddel <- cluster_walktrap(node_deletion_of_keyplayers) # !MANUAL replace cluster_walktrap(node_deletion_of_keyplayers) with cluster_infomap(node_deletion_of_keyplayers) if graph is directed.

# Check modularity score

modularity(node_deletion_of_keyplayers, membership(clustereddel))

# Plot remaining communities (If you wish, may not be the most relevant question after deletions)

# Saving cluster membership into a dataframe

membership_vec <- data.frame(node = V(node_deletion_of_keyplayers)$name, community = clustereddel$membership)

V(node_deletion_of_keyplayers)$community <- clustereddel$membership
...
components(igraphobject)$no #Number of components left( isolated nodes and any seperated communities)
components(igraphobject)$csize %>% max() / gorder(igraphobject)

# Post deletion giant component size
components(node_deletion_of_keyplayers)$no #Number of components left( isolated nodes and any seperated communities)
components(node_deletion_of_keyplayers)$csize %>% max() / gorder(node_deletion_of_keyplayers)

# Original modularity score

modularity(igraphobject, membership(clustered))

# Post deletion modularity score
...
top5_nodes <- names(sort(degrees, decreasing = TRUE))[1:5]  # !MANUAL Decide how many top nodes you want


# Simulate deletion of highest centrality nodes in a new igraph object
node_deletion_of_highdegree <- delete_vertices(igraphobject, top5_nodes)
layouts <- layout_with_fr(node_deletion_of_highdegree)
...
# Determine remaining communities to calculate modularity
clustereddel <- cluster_walktrap(node_deletion_of_highdegree) # !MANUAL replace cluster_walktrap(node_deletion_of_highdegree) with cluster_infomap(node_deletion_of_highdegree) if graph is directed.
...
membership_vec <- data.frame(node = V(node_deletion_of_highdegree)$name, community = clustereddel$membership)

V(node_deletion_of_highdegree)$community <- clustereddel$membership
...
# Post deletion giant component size
components(node_deletion_of_highdegree)$no #Number of components left( isolated nodes and any seperated communities)
components(node_deletion_of_highdegree)$csize %>% max() / gorder(node_deletion_of_highdegree)
...
modularity(node_deletion_of_highdegree, membership(clustereddel))
